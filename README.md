...
Deploy the repo directly on Vercel. `vercel.json` routes every request to `api/index.js`, while `node-server.js` is used for local Node development with `npm start`.
